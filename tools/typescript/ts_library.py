...
def main(argv):
  ...
  with io.open(tsconfig_base_file, encoding='utf-8', mode='r') as f:
    ...
    is_base_tsconfig = args.tsconfig_base is None or \
        args.tsconfig_base.endswith('/tools/typescript/tsconfig_base.json') or \
        args.tsconfig_base.endswith('/tools/typescript/tsconfig_base_react.json')
  ...
  default_jsxImportSource = 'react/jsx-runtime'

  if args.in_files is not None:
    is_ui_lib = 'ui.ts' in args.in_files
    if is_ui_lib:
      react_file_root = 'src/deps/react'
      react_abs_path = os.path.join(args.gen_dir, react_file_root + '.js')

      for f in args.in_files:
        if f in ('ui.ts', react_file_root + '.ts'):
          continue

        [pathname, extension] = os.path.splitext(f)

        in_file_path = os.path.join(args.gen_dir, pathname + '.js')
        ...
        # Find the relative path from the in file to the react.js file.
        in_file_path_dir = os.path.dirname(in_file_path)
        replacement_jsxImportSource = os.path.relpath(react_abs_path, in_file_path_dir)
    ...
    depends_on_ui = any('/ui' in p for p in (args.deps or []))
    if depends_on_ui:
      replacement_jsxImportSource = '//resources/ui/ui.rollup.js'

      for f in args.in_files:
        [pathname, extension] = os.path.splitext(f)

        in_file_path = os.path.join(args.gen_dir, 'tsc', pathname + '.js')
        if not os.path.exists(in_file_path):
          continue
